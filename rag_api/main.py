...
def getLLM(m=settings.LLM_MODEL, t=0.5):
    """Returns global LLM for the process.
    Parameters:
    - m: model to load (default to settings)
    - t: temperature of model (default 0.5)
    """
    return Ollama(
            base_url=settings.OLLAMA_HOST, model=m, verbose=True, temperature=t
    )
...
@guardrail_api
async def context_retrieval(q: str, k:int = settings.CONTEXT_DOCUMENTS_RETRIEVED, h: bool = False):
    ...
    context_retrieval_chain = (
        retriever_with_scores(VECTOR_STORE, k=k)
...
@app.get("/llm")
async def llm(llm_prompt: RAGPrompt, p: int = 0, t: int = 0.5, m = settings.LLM_MODEL):
    """Retrieves response generated by LLM.
    parameters:
    - p: the llm rag prompt to use, defaults to 0.
    """
    ...
    # Guard Statement:
    if p > len(rag_text_prompts) - 1:
        raise Exception(f"Invalid llm prompt selected {p} of {len(rag_text_prompts)}.")

    # Initial LLM generation prompt:
    qa_prompt = PromptTemplate(
        template=rag_text_prompts[p], input_variables=["question", "context"]
    )

    chain = qa_prompt | getLLM( t=t, m=m)
...
@app.get("/answer")
@guardrail_api
async def answer(q: str, h: bool = True, e: bool = True, lp: int = 0, k: int = settings.CONTEXT_DOCUMENTS_RETRIEVED, t = 0.5, m = settings.LLM_MODEL ):
    ...
    # Parameters:
    #   q = question text to answer
    #   h = true/false to enable hyde
    #   e = true/false to enable evaluation
    #   lp = [0..] llm prompt to use
    #   k = num of context to retrieve

    # Context Retrieval
    try:
        context_response = (await context_retrieval(q, k=k, h=h))["response"]
    except Exception as err:
        logger.error(f"Error during context retrieval: {err}")
        logger.error(await context_retrieval(q, k=k, h=h))
    ...
    llm_response = (await llm(prompt, p=lp))["response"]
