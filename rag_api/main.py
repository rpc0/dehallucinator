...
def getLLM(m=settings.LLM_MODEL, t=0.5):
    ...
    """
    llm = Ollama(base_url=settings.OLLAMA_HOST, model=m, verbose=True, temperature=t)
    return llm


async def loadLLM():
    """Load LLM model."""
    # Pre-Load the LLM onto the Ollama server
    try:
...
async def context_retrieval(
    q: str, k: int = settings.CONTEXT_DOCUMENTS_RETRIEVED, h: bool = False
):
...
async def answer(
    ...
    e: bool = True,
    lp: int = 0,
    k: int = settings.CONTEXT_DOCUMENTS_RETRIEVED,
    t=0.5,
